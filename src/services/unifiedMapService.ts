...
class UnifiedMapService {
  ...
  async calculateRoute(origin: string, destination: string): Promise<RouteResponse | null> {
    ...
    try {
      ...
      // Fallback: se a API HERE falhar, usamos a geometria direta
      if (!geometry.length) {
        geometry.push({ lat: originCoords.lat, lng: originCoords.lng });
        geometry.push({ lat: destCoords.lat, lng: destCoords.lng });
      }
